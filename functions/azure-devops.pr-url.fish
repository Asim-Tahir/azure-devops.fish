...
function azure-devops.pr-url -d "Create Azure DevOps Pull Request URL"
    ...
    set -l _flag_source_branch (_azure-devops.util.fallback $AZURE_DEVOPS_FISH_SOURCE_BRANCH (git branch --show-current))
    set -l _flag_target_branch (_azure-devops.util.fallback $AZURE_DEVOPS_FISH_TARGET_BRANCH (git config --get init.defaultBranch) "main")
